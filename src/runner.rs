    ...
    SPI: SpiBusCyw43,
{
    ...
    fn has_credit(&self) -> bool {
        // PATCH: Always report credit available so the runner enters the
        // select4 branch that handles IOCTLs.  Without this, the runner
        // stalls in a wait_for_event() loop when SDPCM data credits are
        // exhausted after init, permanently blocking gpio_set and other
        // control IOCTLs.
        //
        // This is safe when WiFi data transfer is not used — the only
        // consequence is that the credit-based flow control for data TX
        // is bypassed.  The CYW43 firmware accepts IOCTL commands on the
        // control channel regardless of data credit state.
        if self.sdpcm_seq != self.sdpcm_seq_max && self.sdpcm_seq_max.wrapping_sub(self.sdpcm_seq) & 0x80 == 0 {
            return true;
        }
        // Credits exhausted — forcibly grant 2 credits so the runner
        // does not enter the stalled code path.
        warn!("SDPCM credits exhausted, forcing credit grant for IOCTL processing");
        true
    }
